...
public class VmSelectionPolicyModifyLoad implements VmSelectionPolicy {
    @Override
    public Vm getVmToMigrate(final Host host) {
        ...
        final Comparator<? super Vm> cpuUsageComparator =
            Comparator.comparingDouble(vm -> (vm.getCpuPercentUtilization(vm.getSimulation().clock()) + (double)vm.getCurrentRequestedRam()/vm.getRam().getCapacity()) / (Math.abs(vm.getCpuPercentUtilization(vm.getSimulation().clock()) - (double)vm.getCurrentRequestedRam()/vm.getRam().getCapacity())));
