...
public final class GoogleTaskUsageTraceReader extends GoogleTraceReaderAbstract<Cloudlet> {
     ...
     * Eason定制方法，返回是否修改利用率
     * */
    private Predicate<TaskUsage> predicate;
    public GoogleTaskUsageTraceReader setPredicate(final Predicate<TaskUsage> predicate) {
    ...
    protected boolean processParsedLineInternal() {
        ...
        /**
         * Eason定制方法：对读取的TaskUsage添加限制条件，判断是否可以生成cloudlet的利用率trace
         * */
        if(!predicate.test(taskUsage)){
            return false;
        }
